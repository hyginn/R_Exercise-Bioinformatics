...
# You see that there is a table called atom in which the elements are vectors of
# the same length - namely the number of atoms in the structure file. And there
# is a matrix of (x, y, z) triplets called xyz. And there is a vector that holds
# sequence, and two tables called helix and sheet that look a lot like our
# feature annotation tables - in fact many of the principles to store this
# structure data are similar to how we constructed our protein database. Let's
# pull out a few values to confirm how selection and subsetting works here:
...
# bonds. Thus we changed our data structure
...
# == Task:
# Note down the answers to these two questions: I may ask you to hand them in
# for credit later in the course:
#   1: Why do you think these distributions are different?
#   2: At what distance do you think H-bonds have the lowest energy?
...
#  In the Sequence.R script, we calculated information values for an MSA and
#  plotted them along a sequence coordinate. It is of course much more
#  informative to view them in the actual 3D structure. Here, we will write
#  these values to a an "attributes" file for Chimera, assign the attributes to
#  the 1BM8 structure and visualize. Open and scan the page:
#  https://www.cgl.ucsf.edu/chimera/docs/ContributedSoftware/defineattrib/defineattrib.html
#

# == Load the information scores:
load(file = "infoValues.RData")
# ... should recreate "apsesMat" and "vI" from the Sequence.R script. If you do
# not have these two objects saved, then you will need to recreate them (begin
# at line 1676 of Sequence.R).

# confirm:
head(apsesMat)

# subset apsesMat to 1BM8 residues

apsesMat["MBP1_SACCE",]

# Note that all residues of the 1BM8 structure are present in our sequence. This is convenient, because it easily allows us to subset columns that are aligned with this sequence, from apsesMat, and from vI

sel <- apsesMat["MBP1_SACCE", ] != "-"

I3D <- vI[sel]   # information scores for residues of the 1BM8 structure

plot(apses$atom$b[apses$calpha], I3D)  # We see the expected trend that residues
                                       # with low B-values tend to be more
                                       # conserved

# === Create an attribute file for Chimera

residueNumbers <- as.character(apses$atom$resno[apses$calpha])
# confirm
length(residueNumbers) == length(I3D)

Iatt <- character()

Iatt[length(Iatt) + 1] <- "#  PDB entry 1BM8 (Mbp1 APSES domain)"
Iatt[length(Iatt) + 1] <- "#  multi APSES domain MSA"
Iatt[length(Iatt) + 1] <- "#  column-wise information scores"
Iatt[length(Iatt) + 1] <- "#  "
Iatt[length(Iatt) + 1] <- "   attribute: i"
Iatt[length(Iatt) + 1] <- "   match mode: 1-to-1"
Iatt[length(Iatt) + 1] <- "   recipient: residues"
for (i in 1:length(I3D)) {
    num <- residueNumbers[i]
    I <- I3D[i]
    Iatt[length(Iatt) + 1] <-sprintf("\t:%s\t%1.3f ", num, I)
}
writeLines(Iatt, "1BM8_MSAinformation_all.txt")

# In Chimera, open the Define Attributes tool via
# Tools -> Structure Analysis -> Define Attributes
# and load this attributes file. When the attributes have been
# loaded, adjust the white (middle colour) slider to approximately the median
# value of information scores, to balance the visual appearance:
median(I3D)

# Click on oK and study the results. How is this plot of Shannon-information
# values different from the sequential plot? Which one makes more sense? How
# would you interpret what you see?
#
# Select the DNA-binding domain, invert the selection and choose Actions ->
# Atoms/Bonds -> hide to undisplay. Do the information scores for the
# DNA-binding domain make sense?  How would you interpret what you see?

# ==============================================================================
#      PART FIVE: PREPARE APSES DOMAIN SEQUENCES FOR HOMOLOGY MODELING
# ==============================================================================

# Preparing the "target" sequence for homology modelling from the multiple
# sequence alignment is easy: we should include all positions of the alignment
# which are not indels in both sequences:
sel <- apsesMat["MBP1_SACCE", ] != "-" | apsesMat["MBP1_SPIPU", ] != "-"

# ... and apply that selection to both the target:
paste(apsesMat["MBP1_SPIPU", sel], collapse = "")

# ... and confirm for the template:
paste(apsesMat["MBP1_SACCE", sel], collapse = "")

# You can simply copy the console output to use in homology modeling.

# ==============================================================================
#      PART SIX: APSES DOMAIN SEQUENCES OF MBP1 ORTHOLOGUES
# ==============================================================================

# To subset apsesMat for domains that are Mbp1 orthologues only, do the
# following:

# all proteins with names starting with "MBP1_"
mbp1Mat <- apsesMat[grep("MBP1_", rownames(apsesMat)), ]

# drop all columns that contain only hyphens
mbp1Mat <- mbp1Mat[ , apply(mbp1Mat, 2, function(x) { any(x != "-") })]

# Note: I often avoid using apply() because I find a for-loop to be more
# explicit and readable, especially for teaching code. This situation however is
# one where an apply() statement seems appropriate to create the required
# logical vector for subsetting. Study the expression carefully. Perhaps
# contrast it to a for-loop that builds a selection vector column by column.

# apply() can make many things quite easy ...
apply(mbp1Mat, 1, function(x) { paste(x, collapse="") })
x <- AAStringSet(apply(mbp1Mat, 1, function(x) { paste(x, collapse="") }))
writeSeqSet(x, format = "ali", blockSize = 60)

# ==============================================================================
#      PART SEVEN: RENUMBERING THE MODEL
# ==============================================================================

# Save your model pdb file in your working directory with the filename SPIPUmodel.PDB

PDB_INFILE      <- "SPIPUmodel.PDB"
PDB_OUTFILE     <- "SPIPUmodel.2.PDB"


# == Read the model pdb file
...
# check result
SPIPUmodel$atom[ , "resno"]
SPIPUmodel$atom[1, ]

# == Write output to file
write.pdb(pdb = SPIPUmodel, file=PDB_OUTFILE)
...
# (No exercises required for now.
# If you want to practice the principles discussed up to now more deeply,
# you can model the SPIPU Swi6 domain.)
